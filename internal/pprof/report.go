...
type Options struct {
	...
	SampleType        string
	SampleUnit        string // Unit for the sample data from the
...
func (rpt *Report) newTrimmedGraph() (g *Graph, origCount, droppedNodes, droppedEdges int) {
	o := rpt.options
	...
	// The call_tree option is only honored when generating visual representations of the call
...
func (rpt *Report) selectOutputUnit(g *Graph) {
	o := rpt.options
	...
	_, minUnit := Scale(minValue, o.SampleUnit, "minimum")
	_, maxUnit := Scale(maxValue, o.SampleUnit, "minimum")
	...
	if minUnit != maxUnit && minValue*100 < maxValue && o.OutputFormat != Callgrind {
		...
		_, unit = Scale(100*minValue, o.SampleUnit, "minimum")
	}
...
func (rpt *Report) newGraph(nodes NodeSet) *Graph {
	o := rpt.options
	...
	formatTag := func(v int64, key string) string {
		return ScaledLabel(v, key, o.OutputUnit)
	}
	...
	switch o.OutputFormat {
	case Raw, List, WebList, Callgrind:
		gopt.ObjNames = true
	}

	return NewGraph(rpt.prof, gopt)
}
...
func printProto(w io.Writer, rpt *Report) error {
	...
	// Apply the sample ratio to all samples before saving the
	if r := o.Ratio; r > 0 && r != 1 {
...
// printTopProto writes a list of the hottest routines in a profile as a proto.
func printTopProto(w io.Writer, rpt *Report) error {
	...
	for i, n := range g.Nodes {
		...
		fv, _ := Scale(flat, o.SampleUnit, o.OutputUnit)
		cv, _ := Scale(cum, o.SampleUnit, o.OutputUnit)
		s := &Sample{
			Location: []*Location{l},
...
func (fm functionMap) findOrAdd(ni NodeInfo) (*Function, bool) {
...
func printTags(w io.Writer, rpt *Report) error {
	...
	tagKeys := make([]*Tag, 0, len(tagMap))
	for key := range tagMap {
		tagKeys = append(tagKeys, &Tag{Name: key})
	}
	tabw := tabwriter.NewWriter(w, 0, 0, 1, ' ', tabwriter.AlignRight)
	for _, tagKey := range SortTags(tagKeys, true) {
		var total int64
		key := tagKey.Name
		tags := make([]*Tag, 0, len(tagMap[key]))
		for t, c := range tagMap[key] {
			total += c
			tags = append(tags, &Tag{Name: t, Flat: c})
		}

		f, u := Scale(total, o.SampleUnit, o.OutputUnit)
		fmt.Fprintf(tabw, "%s:\t Total %.1f%s\n", key, f, u)
		for _, t := range SortTags(tags, true) {
			f, u := Scale(t.FlatValue(), o.SampleUnit, o.OutputUnit)
			if total > 0 {
...
// printComments prints all freeform comments in the
...
// printText prints a flat text report for a
...
// printTraces prints all traces from a
func printTraces(w io.Writer, rpt *Report) error {
	...
	_, locations := CreateNodes(prof, &GraphOptions{})
...
func printCallgrind(w io.Writer, rpt *Report) error {
	...
	for _, n := range g.Nodes {
		...
		sv, _ := Scale(n.FlatValue(), o.SampleUnit, o.OutputUnit)
		fmt.Fprintf(w, "%s %d %d\n", addr, n.Info.Lineno, int64(sv))

		// Print outgoing edges.
		for _, out := range n.Out.Sort() {
			c, _ := Scale(out.Weight, o.SampleUnit, o.OutputUnit)
			callee := out.Dest
...
func getDisambiguatedNames(g *Graph) map[*Node]string {
	nodeName := make(map[*Node]string, len(g.Nodes))
	...
	// disambiguated name.
	nameFunctionIndex := make(map[names]map[*Node]int)
	for _, n := range g.Nodes {
...
func GetDOT(rpt *Report) (*Graph, *DotConfig) {
...
func printDOT(w io.Writer, rpt *Report) error {
	g, c := GetDOT(rpt)
	ComposeDot(w, g, &DotAttributes{}, c)
	return nil
}

// ProfileLabels returns printable labels for a
func ProfileLabels(rpt *Report) []string {
	...
	if prof.DurationNanos != 0 {
		duration := Label(prof.DurationNanos, "nanoseconds")
		totalNanos, totalUnit := Scale(rpt.total, o.SampleUnit, "nanoseconds")
		var ratio string
...
func reportLabels(rpt *Report, shownTotal int64, nodeCount, origCount, droppedNodes, droppedEdges int, fullHeaders bool) []string {
	...
	label = append(label, fmt.Sprintf("Showing nodes accounting for %s, %s of %s total", rpt.formatValue(shownTotal), strings.TrimSpace(Percentage(shownTotal, rpt.total)), rpt.formatValue(rpt.total)))
...
func New(prof *Profile, o *Options) *Report {
	format := func(v int64) string {
		if r := o.Ratio; r > 0 && r != 1 {
			fv := float64(v) * r
			v = int64(fv)
		}
		return ScaledLabel(v, o.SampleUnit, o.OutputUnit)
	}
...
// available.
func NewDefault(prof *Profile, options Options) *Report {
...
func computeTotal(prof *Profile, value, meanDiv func(v []int64) int64) int64 {
