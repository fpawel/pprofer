...
// data.
func ScaleProfiles(profiles []*Profile) error {
	if len(profiles) == 0 {
		return nil
	}
	periodTypes := make([]*ValueType, 0, len(profiles))
	...
	sampleType := make([]*ValueType, numSampleTypes)
	for i := 0; i < numSampleTypes; i++ {
		sampleTypes := make([]*ValueType, len(profiles))
...
// types.
func CommonValueType(ts []*ValueType) (*ValueType, error) {
	if len(ts) <= 1 {
		return nil, nil
	}
	minType := ts[0]
