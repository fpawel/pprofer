...
func tagDistance(t, u *Tag) float64 {
	v, _ := Scale(u.Value, u.Unit, t.Unit)
...
func (b *builder) tagGroupLabel(g []*Tag) (label string, flat, cum int64) {
	if len(g) == 1 {
		t := g[0]
		return Label(t.Value, t.Unit), t.FlatValue(), t.CumValue()
	}
	min := g[0]
	max := g[0]
	...
	for _, t := range g[1:] {
		if v, _ := Scale(t.Value, t.Unit, min.Unit); int64(v) < min.Value {
			min = t
		}
		if v, _ := Scale(t.Value, t.Unit, max.Unit); int64(v) > max.Value {
			max = t
		}
		f += t.Flat
		df += t.FlatDiv
	...
	return Label(min.Value, min.Unit) + ".." + Label(max.Value, max.Unit), f, c
}
